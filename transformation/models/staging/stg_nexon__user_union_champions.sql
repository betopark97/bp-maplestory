with user_union_champion as (
    ...
    from {{ source('nexon', 'user_union_champion') }}
),

-- Scalars + single-object (error) columns, 1:1 per (snapshot_date, ocid). No list columns.
...
stg_nexon__user_union_champions as (
    select
        spine.snapshot_date,
        spine.ocid,
        spine.error__name,
...
select * from stg_nexon__user_union_champions

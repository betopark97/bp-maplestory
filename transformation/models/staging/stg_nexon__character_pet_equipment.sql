with character_pet_equipment as (
    select * from {{ source('nexon', 'character_pet_equipment') }}
),
...
-- cash_item_option in stg_nexon__character_cash_item_equipment).
...
stg_nexon__character_pet_equipment as (
    select *
    from spine
    left join all_lists using (date, ocid)
)

select * from stg_nexon__character_pet_equipment

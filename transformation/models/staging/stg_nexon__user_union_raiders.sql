with user_union_raider as (
    ...
    from {{ source('nexon', 'user_union_raider') }}
),

-- One row per (snapshot_date, ocid): the scalar spine every list section hangs off.
spine as (
    select
        snapshot_date,
        ocid,
...
union_inner_stat as (
    ...
    full join union_raider_preset_1__union_inner_stat using (snapshot_date, ocid, union_inner_stat__stat_field_id)
    full join union_raider_preset_2__union_inner_stat using (snapshot_date, ocid, union_inner_stat__stat_field_id)
    full join union_raider_preset_3__union_inner_stat using (snapshot_date, ocid, union_inner_stat__stat_field_id)
    full join union_raider_preset_4__union_inner_stat using (snapshot_date, ocid, union_inner_stat__stat_field_id)
    full join union_raider_preset_5__union_inner_stat using (snapshot_date, ocid, union_inner_stat__stat_field_id)
),
...
stg_nexon__user_union_raiders as (
    select
        snapshot_date,
        ocid,
        use_preset_no,
...
select * from stg_nexon__user_union_raiders

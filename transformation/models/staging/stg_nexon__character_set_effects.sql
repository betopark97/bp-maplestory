with character_set_effect as (
    ...
    from {{ source('nexon', 'character_set_effect') }}
),
...
stg_nexon__character_set_effects as (
    select
        snapshot_date,
        ocid,
        set_effect__set_name,
    ...
    full join set_effect_info as sei using (snapshot_date, ocid, set_effect__set_name, set_effect__set_count)
)

select * from stg_nexon__character_set_effects

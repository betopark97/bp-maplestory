with user_union_artifact as (
    select * from {{ source('nexon', 'user_union_artifact') }}
),

-- Scalars + single-object columns, 1:1 per (date, ocid). No list columns.
spine as (
    select
        date,
        ocid,
...
-- List 2: union_artifact_crystal {name, validity_flag, date_expire, level,
...
union_artifact_crystal as (
    select
        ...
        crystal.el ->> 'validity_flag'                 as union_artifact_crystal__validity_flag,
...
all_lists as (
    select
        ...
        null::text          as union_artifact_crystal__validity_flag,
    ...
    select
        ...
        union_artifact_crystal__validity_flag,
...
stg_nexon__user_union_artifact as (
    select
        spine.date,
        spine.ocid,
        ...
        all_lists.union_artifact_crystal__validity_flag,
...
select * from stg_nexon__user_union_artifact

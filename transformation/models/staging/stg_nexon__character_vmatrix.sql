with character_vmatrix as (
    select * from {{ source('nexon', 'character_vmatrix') }}
),
...
v_core_equipment as (
    ...
    from vce
    full join vcep1 using (date, ocid, slot_id, v_core_type)
    full join vcep2 using (date, ocid, slot_id, v_core_type)
    full join vcep3 using (date, ocid, slot_id, v_core_type)
    full join vcep4 using (date, ocid, slot_id, v_core_type)
    full join vcep5 using (date, ocid, slot_id, v_core_type)
),

stg_nexon__character_vmatrix as (
    select
        cvm.date,
        cvm.ocid,
        cvm.character_class,
    ...
    left join v_core_equipment as vce
        on cvm.ocid = vce.ocid and cvm.date = vce.date
)

select * from stg_nexon__character_vmatrix
